...
if(isset($_GET['search'])){
        ...
        $query2 ="SELECT D.*, C.* FROM dvd D LEFT OUTER JOIN category C ON D.CategoryId = C.CategoryId WHERE (Name LIKE '%{$search}%'AND  Is_deleted='0'AND Is_TV_Series='1' AND Quantity>'5') ORDER BY Name";
    }
    

// else if(isset($_POST['movie'])){
//     $query2 ="SELECT D.*, C.* FROM dvd D LEFT OUTER JOIN category C ON D.CategoryId = C.CategoryId WHERE (Is_Movie='1'AND C AND Is_deleted='0' AND Quantity>'5')ORDER BY Name";
// }
...
$query2 ="SELECT D.*, C.* FROM dvd D LEFT OUTER JOIN category C ON D.CategoryId = C.CategoryId WHERE (Is_deleted='0' AND Is_TV_Series='1' AND Quantity>'5') ORDER BY Name";
}
    ...
    if( $dvd2){
      while($movie2 = mysqli_fetch_assoc($dvd2)){
         ...
         $movielist .= "<td><a href=\"modifytv.php?Id={$movie2['DVD_Id']}\">Edit</a></td>" ;  

         $movielist .= "<td><a href=\"deletetv.php?Id={$movie2['DVD_Id']}\"onclick =\"return confirm('Are you sure to remove movie?');\" >Delete</a></td>" ;       
...
<body>
  ...
  <div class="contentBx3"> 
       ...
       <form action="tvlist.php" method="get">
        <div class="container6">
            &ensp;  <h3 class="titleText" ><a href="TVseriesForm.php" id="link" class="btn"> +Add New </a> <a href="tvlist.php" class="btn"> Refresh </a>  </h3>
            <div class="row100">
                <div class="col">
                    <div class="inputBox">
                   <input type ="text" name ="search" id="" placeholder="Type  Name  and Press Enter"  value= "<?php echo $search;?>"  style="width :99%;"required  autofocus>
