...
if (isset($_GET['RefNo'])) {
    ...
    if ($result_set) {
        if (mysqli_num_rows($result_set) == 1) {
            ...
            // user not found
            header('Location: adminorders-view.php?err=user_not_found');
        }
    } else {
        // query unsuccessful
        header('Location: adminorders-view .php?err=query_failed');
    }
...
if(isset($_POST['submit']))
{
    ...
    if ($result){
        if($pay_method == 'lending'){
            ...
            $query3 = "INSERT INTO lend(RefNo,IssueDate,ReturnDate,Total_Cost,UserName,Customer_email,ODescription,CustomerId,Confirmation)
              VALUES('{$RefNo}','{$Order_date}','{$ReturnDate}',{$cost},'{$User_Name}','{$Email}','{$Description}','{$CusId}',1)";
            $result1 = mysqli_query($connection , $query3);

            if( $result1){

                header('Location:adminorders-view.php ? order_confirmed=true');
            }

        }else {

            header('Location:adminorders-view.php ? order_confirmed=failed');
        }
    }
    else{
...
<section class="banner3">
    ...
    <header>
        ...
        <ul class="navigation">
            ...
            <li><a href="adminorders-view.php">Back</a></li>
    ...
    <form action ="adminconfirm_orders.php" method ="post">
        ...
        <div class="container6">
            ...
            <input type ="hidden" name="Payment_method"  value="<?php echo $pay_method ;?>">
            <input type ="hidden" name="Descrition"  value="<?php echo $Description ;?>">
